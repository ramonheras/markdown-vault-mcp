...
1. Call `search` with that query and mode='hybrid'. If the call fails (semantic search not configured), retry with mode='keyword'. Examine the top results; call `read` on the 3-5 highest-scoring paths.
