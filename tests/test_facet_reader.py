...
class TestReaderFacetAccessor:
    def test_accessor_returns_reader_facet(self, built: Vault) -> None:
...
class TestReaderFacetBehaviour:
    def test_search_returns_list(self, built: Vault) -> None:
        assert isinstance(built.reader.search("frontmatter"), list)

    def test_read_existing_document(self, built: Vault) -> None:
    ...
    def test_stats_returns_vault_stats(self, built: Vault) -> None:
        stats = built.reader.stats()
        assert isinstance(stats, VaultStats)
        assert stats.document_count >= 1

    def test_get_history_empty_without_git(self, built: Vault) -> None:
        # vault fixture is not a git repo -> git strategy is None -> [].
        assert built.reader.get_history() == []

    def test_get_diff_empty_without_git(self, built: Vault) -> None:
