...
Exercises the writer facet through a real Collection (real on-disk effects),
mirroring the flat-method assertions in test_collection.py.
"""
...
@pytest.fixture
def writable(vault_path: Path) -> Iterator[Collection]:
    """Writable, indexed collection accepting any attachment extension."""
    col = Collection(
...
class TestWriterFacetAccessor:
    def test_accessor_returns_writer_facet(self, writable: Collection) -> None:
        assert isinstance(writable.writer, WriterFacet)

    def test_accessor_is_stable(self, writable: Collection) -> None:
        assert writable.writer is writable.writer


class TestWriterFacetBehaviour:
    def test_write_creates_document(
    ...
    def test_edit_patches_document(
        self, writable: Collection, vault_path: Path
    ) -> None:
